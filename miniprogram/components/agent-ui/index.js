...
Component({
  ...
  methods: {
    ...
    sendMessage: async function (message) {
      ...
      // 检查是否有书籍信息，如果有，添加到消息中（但用户不会看到）
      let actualMessage = inputValue;
      if (this.data.bookInfo) {
        // 创建一个更自然的前缀，以特定背景引导AI回答问题
        const bookInfoPrefix = `假如我是一名${this.data.bookInfo.zy}的一名大学生，当前学习的课程是${this.data.bookInfo.course}（教材名称：${this.data.bookInfo.book_name}）。请结合我当前这个背景，回答我下列的问题。\n\n`;
        // 实际发送给AI的消息将包含书籍信息
        actualMessage = bookInfoPrefix + inputValue;
        console.log('发送消息包含书籍信息:', actualMessage);
      }

      const chatMode = this.data.chatMode;
      // console.log(inputValue,bot.botId)
      const userRecord = {
        content: inputValue, // 用户界面显示的仍然是原始消息
        record_id: "record_id" + String(+new Date() - 10),
        role: "user",
    ...
    setBookInfo: function(bookInfo) {
      console.log('agent-ui组件接收到书籍信息:', bookInfo);
      this.setData({
        bookInfo: bookInfo
      });
    },
